...
<%
    ...
    String preDescription     = (String) request.getAttribute("preDescription");
    String preBrand           = (String) request.getAttribute("preBrand");
    String preModel           = (String) request.getAttribute("preModel");
    String preConditionLevel  = (String) request.getAttribute("preConditionLevel");
    String preFlawDesc        = (String) request.getAttribute("preFlawDesc");
    String preImageUrls       = (String) request.getAttribute("preImageUrls");
    String preStartPrice      = (String) request.getAttribute("preStartPrice");
    String preBidIncrement    = (String) request.getAttribute("preBidIncrement");
    String preReservePrice    = (String) request.getAttribute("preReservePrice");
    String preStartTime       = (String) request.getAttribute("preStartTime");
    String preEndTime         = (String) request.getAttribute("preEndTime");
    Integer preCategoryId     = (Integer) request.getAttribute("preCategoryId");

    if (preStartTime == null || preStartTime.isEmpty()) {
        java.time.LocalDateTime now = java.time.LocalDateTime.now().plusMinutes(1);
        preStartTime = now.format(java.time.format.DateTimeFormatter.ofPattern("yyyy-MM-dd'T'HH:mm"));
    }
    if (preEndTime == null || preEndTime.isEmpty()) {
        java.time.LocalDateTime end = java.time.LocalDateTime.now().plusDays(3);
        preEndTime = end.format(java.time.format.DateTimeFormatter.ofPattern("yyyy-MM-dd'T'HH:mm"));
    }
%>
<!DOCTYPE html>
<html lang="zh-CN">
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .cp-card-sub span.required { color: #ff00ff; text-shadow: 0 0 5px #ff00ff; }
        ...
        .cp-cover-preview .ph-icon { font-size: 28px; color: rgba(255,238,0,0.2); }
        .cp-cover-preview .ph-text { display: block; margin-top: 4px; }
        ...
        .cp-image-thumb:hover .cp-thumb-actions { opacity: 1; }
        .cp-thumb-btn {
            width: 26px; height: 26px; padding: 0;
            border: 1px solid #FFEE00; cursor: pointer;
            background: #0a0a0a; color: #FFEE00;
            font-size: 12px; display: grid; place-items: center;
            font-family: inherit;
        }
        .cp-thumb-btn:hover { background: #FFEE00; color: #0a0a0a; }
        .cp-thumb-btn.danger:hover { background: #ff00ff; color: #0a0a0a; border-color: #ff00ff; }
        .cp-thumb-btn.cover-tag {
            background: #00f0ff; color: #0a0a0a; cursor: default; border-color: #00f0ff;
        }
        .cp-thumb-btn.cover-tag:hover { background: #00f0ff; color: #0a0a0a; }

        /* 上传按钮条 */
        .cp-upload-bar { display: flex; align-items: center; gap: 8px; }
        .cp-upload-btn {
        ...
        .cp-upload-btn:has(input:disabled) { opacity: 0.6; cursor: not-allowed; }
...
<div class="cp-container-sm">
    <div id="app" class="cp-card">
        ...
        <div class="cp-card-header">
            <div class="cp-card-title glitch" data-text="发布拍品">
                <span class="cp-card-title-icon"><i class="fa fa-plus"></i></span>
                <span>发布拍品</span>
            </div>
            <p class="cp-card-sub">填写下面的信息，把您的闲置变成价值。所有带 <span style="color: #ff00ff; text-shadow: 0 0 5px #ff00ff;">*</span> 的为必填项。</p>
        </div>

        <div v-if="serverError" class="cp-alert cp-alert-error">{{ serverError }}</div>

        <form action="<%=ctx%>/item?action=publish" method="post"
            ...
            <div class="cp-section">
                ...
                <div class="cp-form-group">
                    ...
                    <select name="categoryId" class="cp-form-input" v-model.number="form.categoryId"
                            :class="{ error: touched.categoryId && errors.categoryId }">
            ...
            <div class="cp-section">
                <div class="cp-section-head">
                    ...
                    <span class="cp-section-tip">每行一个 URL，最多 5 张，第一行作为封面</span>
                </div>
                ...
                <div class="cp-form-row-3">
                    <div class="cp-form-group">
                        ...
                        <input type="number" name="startPrice" class="cp-form-input" min="0.01" step="0.01"
                               ...
                               :class="{ error: touched.startPrice && errors.startPrice }">
                        <div class="cp-quick-row">
                    ...
                    <div class="cp-form-group">
                        ...
                        <input type="number" name="bidIncrement" class="cp-form-input" min="0.01" step="0.01"
                               ...
                               :class="{ error: touched.bidIncrement && errors.bidIncrement }">
                        <div class="cp-quick-row">
                            <span class="cp-quick-row-label">快捷：</span>
                ...
                <div class="cp-form-row">
                    <div class="cp-form-group">
                        ...
                        <input type="datetime-local" name="startTime" class="cp-form-input"
                               ...
                               :class="{ error: touched.startTime && errors.startTime }">
                    ...
                    <div class="cp-form-group">
                        ...
                        <input type="datetime-local" name="endTime" class="cp-form-input"
                               ...
                               :class="{ error: touched.endTime && errors.endTime }">
                        <div class="cp-quick-row">
            ...
            <div class="cp-submit-bar">
                ...
                <a href="<%=ctx%>/item?action=list" class="cp-btn-outline cp-btn-sm">取消</a>
                ...
                setup() {
                    ...
                    function handleSubmit(e) {
                        ...
                        if (failed.length > 0) {
                            ...
                            toast('请补全/修正以下字段：\n' + detail, 'error');
                            // 同时 console 打印详细状态，便于调试
                            console.warn('[publish] 校验失败：', failed, '\n当前 form 状态：', JSON.parse(JSON.stringify(form)));
                            return;
                        }
